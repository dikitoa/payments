...
public class CreateCardValidator implements Validator {
	...
	public void validate(Object object, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "commissionMaintenance", "required.commissionMaintenance", "Commission Maintenance field is necesary.");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "commissionRenovate", "required.commissionRenovate", "Commission Renovate field is necesary.");
		...
		validateCommissionEmission(bean.getCommissionEmission(), errors);
		validateCommissionMaintenance(bean.getCommissionMaintenance(), errors);
		validateCommissionRenovate(bean.getCommissionRenovate(), errors);
	...
	private void validateCommissionEmission(double commissionEmission, Errors errors) {
		if (!errors.hasFieldErrors("commissionEmission")) {
			if (commissionEmission >= Double.MAX_VALUE) {
				errors.rejectValue("commissionEmission", "max.commissionEmission");
			} 
		}
	}

	private void validateCommissionMaintenance(double commissionMaintenance, Errors errors) {
		if (!errors.hasFieldErrors("commissionMaintenance")) {
			if (commissionMaintenance >= Double.MAX_VALUE) {
				errors.rejectValue("commissionMaintenance", "max.commissionMaintenance");
			} 
		}
	}

	private void validateCommissionRenovate(double commissionRenovate, Errors errors) {
		if (!errors.hasFieldErrors("commissionRenovate")) {
			if (commissionRenovate >= Double.MAX_VALUE) {
				errors.rejectValue("commissionRenovate", "max.commissionRenovate");
			} 
		}
	}
