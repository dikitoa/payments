...
public class Enterprise extends Client{
    ...
    public Enterprise(char cifLetter, int cifNumber, char cifControl) throws MalformedHandlerException {
        super(new EnterpriseHandler(cifLetter, cifNumber, cifControl).toString());
    ...
    public Person removeAuthorizedPerson(Handler personHandler){
        ...
        while(iterator.hasNext()){
            ...
            if(person.getId().compareTo(personHandler.toString())==0){
                result=person;
     ...
     * @return 
     */
    public boolean existsAuthorizedPerson(String string){
        boolean result=false;
        Iterator<Person> iterator = authorizedPersons.iterator();
        while(iterator.hasNext()){
            Person person = iterator.next();
            if(person.getId().compareTo(string)==0)
                result=true;
        }
        return result;
    }
