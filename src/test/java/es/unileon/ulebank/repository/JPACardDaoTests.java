...
public class JPACardDaoTests {
    ...
    public void testSaveProduct() throws InvalidFeeException, CommissionException {
		...
		Card testCard = new CreditCard(handler.toString(), client, account, 400.0, 1000.0, 400.0, 1000.0, commissionEmission.getFee(0), commissionMaintenance.getFee(0), commissionRenovate.getFee(0));

        cardDao.addCard(testCard);

        Card testCardCopy = cardDao.getCardDAO(testCard.getId());
        
        assertEquals(testCardCopy.getId(), testCard.getId());
    }
