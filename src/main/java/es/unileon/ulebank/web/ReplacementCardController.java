 ...
 */
@Controller
@RequestMapping ("/replacementcard.htm")
public class ReplacementCardController {
	...
	public ModelAndView submit(@Valid CardBean bean, BindingResult result) throws Exception  {
		if (result.hasErrors()) {
			return new ModelAndView("replacementcard", "model", bean);
		}
		Cards card = this.cardManager.findCard(id);
		
		Command replacementCard = new ReplacementCardCommand(card);
		replacementCard.execute();
