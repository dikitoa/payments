...
public class ChargeCreditCardPaymentsCommand implements Command {
	 ...
	 * @throws IOException 
	 */
	public ChargeCreditCardPaymentsCommand(Handler id, CreditCard card, Date effectiveDate) throws IOException{
	...
	public void execute() throws InvalidFeeException, PaymentException,
		...
		try{
			this.card.getAccount().doWithdrawal(new CardTransaction(this.card.getAmount(this.effectiveDate),this.effectiveDate, this.executeConcept + this.card.getCardNumber().toString()));
		} catch (TransactionException e) {
			LOG.info(e.getMessage());
			throw new TransactionException("Can't done transaction of charge amount of purchases with credit card " + this.card.getCardNumber().toString());
		}
	}
	...
	public void undo() throws TransferException, TransactionException,
			IOException {
		try{
			this.card.getAccount().doDeposit(new CardTransaction(this.card.getAmount(this.effectiveDate),this.effectiveDate, this.undoConcept + this.card.getCardNumber().toString()));
		} catch (TransactionException e) {
			LOG.info(e.getMessage());
			throw new TransactionException("Can't done transaction of repayment the amount of purchases with credit card " + this.card.getCardNumber().toString());
		}
	}
